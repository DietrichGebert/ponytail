...
| Goose | `skills/`, `AGENTS.md` | Skills-tier via the open `.agents/skills/` locations (global `~/.agents/skills/`, project `.agents/skills/`); goose turns each installed skill into a slash command, so `/ponytail`, `/ponytail-review`, etc. work as-is. Always-on rules ride the `AGENTS.md`/`.goosehints` context files (`~/.agents/AGENTS.md` global, repo root per project). No manifest to ship (goose has no plugin marketplace) and no hooks (goose has no lifecycle hooks). |
