...
| Factory Droid | `.factory-plugin/plugin.json`, `.factory-plugin/marketplace.json`, `hooks/factory-hooks.json`, `hooks/`, `skills/` | Claude Code plugin compatible. `droid plugin marketplace add DietrichGebert/ponytail` + `droid plugin install ponytail@ponytail`. Hooks use `${DROID_PLUGIN_ROOT}` and set `PONYTAIL_HOST=factory`; state lives under `~/.factory`. Instruction-only fallback: copy `AGENTS.md` + `skills/` into `~/.factory/`. |
