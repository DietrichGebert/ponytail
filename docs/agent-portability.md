...
| Kimi Code | `hooks/kimi-code-hooks.toml`, `hooks/`, `skills/`, `AGENTS.md` | Merge the `[[hooks]]` block from `hooks/kimi-code-hooks.toml` into `~/.kimi-code/config.toml`: `UserPromptSubmit` activates the default mode, tracks `/ponytail` switches, and injects the ruleset every turn (a hook's stdout is appended to the model context on exit 0). Subagent injection is unavailable in kimi 0.38–0.40.1 (`PreToolUse` stdout discarded, `SubagentStart` observation-only); a commented-out block ships in the template for when that changes. Copy the six `skills/` directories into `~/.kimi-code/skills/` for the commands. Reads `AGENTS.md` from the project root as instruction-tier fallback. |
