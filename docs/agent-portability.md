...
| Kiro | `.kiro/steering/ponytail.md`, `hooks/kiro-hooks.json`, `hooks/` | Instruction-tier works with zero setup: copy `.kiro/steering/ponytail.md` globally (`~/.kiro/steering/`) or into a project for the always-on ruleset. Plugin-tier: copy `hooks/kiro-hooks.json` into a `.kiro/hooks/ponytail.json` (replace `PONYTAIL_DIR`) to register `SessionStart` (activation) and `UserPromptSubmit` (`/ponytail` level switches). The `--host=kiro` flag routes the shared `hooks/*.js` scripts to Kiro's raw-stdout output and its own `~/.kiro` state dir. No subagent-injection hook: Kiro has no `SubagentStart` event, and a `PreToolUse` hook's stdout is ignored on exit 0 (it can only block via stderr), so the always-on steering rule covers agents instead. |
