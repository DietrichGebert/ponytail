...
| JetBrains Junie | `AGENTS.md` | Junie reads `AGENTS.md` from the project root, which this repo ships, so it works in any JetBrains IDE with no setup; `~/.junie/AGENTS.md` makes it global, and `.junie/guidelines.md` is Junie's legacy path. Instruction-tier. |
