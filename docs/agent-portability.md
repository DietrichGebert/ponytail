...
| Grok Build | root `plugin.json` → shared `hooks/claude-codex-hooks.json`, `.grok-plugin/marketplace.json`, `hooks/`, `skills/` | `grok plugin install DietrichGebert/ponytail --trust`, then enable. Reuses the Claude/Codex hook map (`CLAUDE_PLUGIN_ROOT` is set as a Grok alias); mode state under `GROK_PLUGIN_DATA`. No root `hooks/hooks.json` (Gemini). |
