...
| Qwen Code | `gemini-extension.json`, `AGENTS.md`, `commands/`, `skills/` | Installs through Qwen's Gemini CLI compatibility, which converts the manifest, preserves `AGENTS.md` as extension context, migrates the TOML commands, and registers the six skills as `ponytail:<skill>`. Claude/Codex lifecycle hooks are not used, so persistent mode tracking is unavailable. |
