...
| Kiro | `.kiro/steering/ponytail.md`, `hooks/kiro-hooks.json`, `hooks/`, `skills/` | Instruction-tier works with zero setup: copy `.kiro/steering/ponytail.md` globally (`~/.kiro/steering/`) or into a project for the always-on ruleset. Plugin-tier: copy `hooks/kiro-hooks.json` into a `.kiro/hooks/ponytail.json` (replace `PONYTAIL_DIR`) to register `SessionStart` (activation), `UserPromptSubmit` (`/ponytail` level switches), and `PreToolUse` matched to the sub-agent tool (subagent injection). The `--host=kiro` flag routes the shared `hooks/*.js` scripts to Kiro's raw-stdout output and its own `~/.kiro` state dir. |
