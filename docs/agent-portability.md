...
| DeepSeek Harness (DSH) | `.dsh-plugin/` (`cordis.patch.yml`, `lib/`) | Native DSH bundle, thin adapter: one cordis patch mounts a zero-dependency Node plugin that registers the six `/ponytail*` commands and a global-layer skill provider reading the repo-root `skills/` directly (the web profile disables the host skill-filesystem row; global providers reach every preset). The npm tarball ships a generated `skills/` copy (prepack via `scripts/sync-skills.mjs`, byte-checked by tests). |
