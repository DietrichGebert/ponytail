...
| CodeBuddy | `.codebuddy-plugin/plugin.json`, `commands/`, `hooks/codebuddy-hooks.json`, `hooks/` | Full plugin install with session activation, mode tracking, commands, and statusline support. Uses its own `codebuddy-hooks.json` mirroring the Claude/Codex hook format. |
