...
| ECA | `.eca-plugin/marketplace.json`, `eca.json`, `skills/`, `hooks/`, `AGENTS.md` | Plugin install by registering the repo as a `plugins` source in `~/.config/eca/config.json`: the six skills become slash commands and auto-invoke on coding tasks, and the `eca.json` config override registers `chatStart` (activation + ruleset into the system prompt), `preRequest` (mode-switch tracking), and `subagentStart` (subagent injection) hooks that run the shared `hooks/` scripts via `${plugin:root}`. Hooks live in `eca.json`, not `hooks/hooks.json`: the Gemini extension auto-loads that path with incompatible event names. From a checkout, ECA reads `AGENTS.md` and discovers the root `skills/` with zero setup. ECA expands skill slash commands and drops placeholder-less args, so level switches go through `$ponytail <level>`, which the `preRequest` hook parses. |
