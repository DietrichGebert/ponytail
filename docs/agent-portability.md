...
| Devin CLI | `.devin-plugin/plugin.json`, `hooks/ponytail-devin.js`, `scripts/devin-cli-setup.sh`, `skills/` | `devin plugins install DietrichGebert/ponytail` adds the skills. Because Devin CLI does not run plugin install scripts, run `scripts/devin-cli-setup.sh` once to copy `ponytail-devin.js` into `~/.config/devin/hooks/` and register `SessionStart` / `UserPromptSubmit` / `PostCompaction` hooks in `~/.config/devin/config.json`. After that, ponytail is active every session and respects `PONYTAIL_DEFAULT_MODE` / `~/.config/ponytail/config.json`. |
