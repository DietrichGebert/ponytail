...
| OpenCode | `.opencode/plugins/ponytail.mjs`, `.opencode/command/`, `hooks/`, `skills/` | Dual V1 + V2 server plugin from one entrypoint (`server()` hooks on V1, `id` + `setup()` on V2: ruleset via a `context` session hook, commands and skills via transforms, one shared mode flag). Reuses the shared instruction builder. |
