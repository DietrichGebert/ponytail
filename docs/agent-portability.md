...
| IBM Bob | `skills/`, `AGENTS.md` | `npx skills add DietrichGebert/ponytail --agent bob --skill '*' --global --yes` copies the six skills into `~/.bob/skills/` (drop `--global` for `<project>/.bob/skills/`). Bob invokes them as `/ponytail`, `/ponytail-review`, etc. or auto-activates from the skill descriptions. `AGENTS.md` at the repo root is the instruction-tier fallback. |
