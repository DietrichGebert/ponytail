...
| OMP (oh-my-pi) | `pi-extension/`, `skills/`, `package.json#omp`, `.claude-plugin/marketplace.json` | `omp plugin marketplace add DietrichGebert/ponytail` + `omp plugin install ponytail@ponytail`. The shared pi extension injects the ruleset each turn through the shared instruction builder and registers the `/ponytail` commands; omp's `omp-plugins` provider discovers the bundled `skills/` next to the package. omp reads the Claude-compatible `.claude-plugin/marketplace.json` as its catalog. |
