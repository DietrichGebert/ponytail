#!/usr/bin/env node
// Pin every version-bearing file to one X.Y.Z, including plugin.yaml. On a
// release-tag CI run the shared version must equal the tag.
...
function readVersion(relPath) {
  try {
    ...
    if (relPath.endsWith('.yaml') || relPath.endsWith('.yml')) {
      ...
      if (!match) throw new Error('missing version: field');
      return match[1];
    }
