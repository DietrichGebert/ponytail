...
test("filterSkillBodyForMode keeps rule bullets that contain a colon", () => {
  ...
  // "No speculative abstractions:" rule or the `ponytail:` comment convention)
  ...
  assert.ok(filtered.includes('full: "Establish the required'));
  assert.ok(!filtered.includes('lite: "Build the requested'));
  assert.ok(!filtered.includes('ultra: "First prove'));
});
