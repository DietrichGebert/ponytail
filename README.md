...
Kiro (plugin-tier): also copy [`hooks/kiro-hooks.json`](hooks/kiro-hooks.json) into `.kiro/hooks/ponytail.json` (project) or `~/.kiro/hooks/ponytail.json` (global) and replace `PONYTAIL_DIR` with the path to your ponytail checkout. This registers `SessionStart` activation, `UserPromptSubmit` `/ponytail` level switching (`lite`/`full`/`ultra`/`off`), and subagent injection via a `PreToolUse` hook matched to Kiro's sub-agent tool. `node` must be on your `PATH`. Kiro forwards each hook's stdout as injected context; the `--host=kiro` flag routes the shared scripts to raw-stdout output and the `~/.kiro` state dir.
