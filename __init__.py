...
def _fallback_instructions(mode: str) -> str:
    return (
        ...
        "No speculative abstractions, avoidable dependencies, boilerplate, or "
        "speculative scaffolding. Deletion over addition. Boring over clever. "
        "A cohesive boundary that keeps policy out of callers is not speculative. "
        "Minimize the concepts, contracts, and places a maintainer must inspect. "
        "Keep unavoidable policy behind one explicit boundary, even when it takes more lines locally. "
        "Prefer plain data and transparent composition over custom behavior with hidden side effects. "
        "Shortest working diff is a tie-breaker after correctness and contained complexity. "
        "Does this simplify the system, or move complexity into callers? "
